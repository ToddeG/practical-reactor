...
public class c7_ErrorHandling extends ErrorHandlingBase {
    ...
    public void potato_potato() {
        Mono<String> currentUser = getCurrentUser()
                .onErrorMap(SecurityException::new)//todo: change this line only
    ...
    public void have_a_backup() {
        ...
        Flux<String> messages = messageNode().doOnError(this::errorReportService)
        ...
        Flux<Task> taskFlux = taskQueue()
                .flatMap(task ->
                        task.execute().then(task.commit())
                                ...
                                .thenReturn(task))//todo: do your changes here
                ;
    ...
    public void billion_dollar_mistake() {
        Flux<String> content = getFilesContent()
                ...
                .onErrorContinue((e, o) -> System.out.println(e.getMessage()))//todo: change this line only
                ;
    ...
    public void back_off() {
        Mono<String> connection_result = establishConnection()
                .retryWhen(Retry.backoff(3, Duration.ofSeconds(5))) //todo: change this line only
                ;
    ...
    public void good_old_polling() {
        //todo: change code as you need
        Flux<String> alerts = nodeAlerts().repeatWhenEmpty(i ->
                i.delayElements(Duration.ofSeconds(1))).repeat();
